...
class BotHandlers:
    ...
    async def check_channel_membership(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
        ...
        try:
            member = await context.bot.get_chat_member("@Sketch_Temp", user_id)
            if member.status in [ChatMember.LEFT, ChatMember.BANNED]:
                return False
        except Exception as e:
            logging.warning(f"Could not check membership for Sketch_Temp: {e}")
...
Please join the channel to unlock AI features\\!"""
    ...
    async def send_welcome_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = [
            [InlineKeyboardButton("🤖 Chat with Fantom AI", callback_data="chat_fantom")],
    ...
    async def send_main_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = [
            [InlineKeyboardButton("🤖 Chat with FANTOM", callback_data="chat_fantom")],
            [InlineKeyboardButton("👨‍💻 Developer", url=f"https://t.me/{DEVELOPER_USERNAME.replace('Sketch_Developer_Pro', '')}")]
        ]
        
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        message = f"🎯 **FANTOM DELUXE AI**\n\nHello {update.effective_user.first_name}! How can I assist you today?"
...
3. Click "Chat with FANTOM" to start AI conversation
4. Ask any questions - I'll respond with AI assistance!

**Developer:** @Sketch_Developer_Pro
"""
